...
2.  Let admins type the encryption key on the VM's console.
...
3.  Let the VM pull the encryption key via instance metadata or instance configuration.
...
4.  Let the VM pull the encryption key from an external location which features encryption-at-rest.
...
5.  Rely on Infrastructure Provider to provide encryption-at-rest.
...
2.  Encrypt data using server-side-encryption.
...
3.  Rely on Infrastructure Provider to provide encryption-at-rest.
