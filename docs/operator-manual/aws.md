...
1. Expose AWS credentials to Terraform.
    ...
    export TF_VAR_AWS_SSH_KEY_NAME="yyy" # Name of the AWS keypair to use for the EC2 instances
...
1. Init the Kubespray config in your config path.
    ...
    ```
    for CLUSTER in $SERVICE_CLUSTER $WORKLOAD_CLUSTERS; do
        ./bin/ck8s-kubespray init $CLUSTER aws /path/to/private/ssh/key
    done
    ```

    !!! important
        The key in `/path/to/private/ssh/key` must be the private key of the key referenced in `TF_VAR_AWS_SSH_KEY_NAME` above.
